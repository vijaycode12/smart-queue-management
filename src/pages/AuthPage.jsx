...
.sq-inp:focus { border-color: #1e1e4a !important; box-shadow: 0 0 0 3px rgba(30,30,74,.12); }

.sq-btn-primary { transition: transform .15s, box-shadow .15s, opacity .15s; }
.sq-btn-primary:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 12px 30px rgba(30,30,74,.45) !important; }
.sq-btn-primary:active:not(:disabled) { transform: translateY(0); }

.sq-link { transition: color .15s; }
.sq-link:hover { color: #1e1e4a !important; text-decoration: underline; }
...
export default function AuthPage({ onLogin }) {
  ...
  return (
    /* Changed: outer bg from #0d0b1e → #f0f0f6 (light gray, matches right panel of screenshot) */
    <div style={{ minHeight: "100vh", background: "#f0f0f6", display: "flex", flexDirection: "column", fontFamily: "'DM Sans', sans-serif" }}>
      {/* Background orbs — toned down for light bg */}
      <div style={{ position: "fixed", inset: 0, overflow: "hidden", pointerEvents: "none", zIndex: 0 }}>
        <div style={{ position: "absolute", width: 600, height: 600, borderRadius: "50%", background: "radial-gradient(circle, rgba(30,30,74,.07) 0%, transparent 70%)", top: "-200px", left: "-100px", animation: "orb1 12s ease infinite" }} />
        <div style={{ position: "absolute", width: 500, height: 500, borderRadius: "50%", background: "radial-gradient(circle, rgba(247,151,30,.06) 0%, transparent 70%)", bottom: "-150px", right: "-100px", animation: "orb2 16s ease infinite" }} />
      </div>

      {/* Navbar — changed from dark to light navy panel */}
      <nav style={{ position: "relative", zIndex: 10, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "22px 48px", background: "#1a1a3e", borderBottom: "1px solid rgba(255,255,255,.08)" }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          {/* Register button: changed gradient → solid dark navy + yellow accent */}
          <button onClick={() => { setView("register"); setError(""); }} className="sq-nav-btn" style={{ padding: "9px 20px", borderRadius: 10, border: "none", background: "linear-gradient(135deg, #ffd200, #f7971e)", color: "#1a1a3e", fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Register</button>
        </div>
      </nav>

      {/* Auth Card — light card on light bg */}
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "40px 20px", position: "relative", zIndex: 5 }}>
        <div className="sq-form-wrap" style={{ width: "100%", maxWidth: 440, background: "#ffffff", border: "1px solid #e2e2ee", borderRadius: 24, padding: "44px 44px 36px", boxShadow: "0 8px 40px rgba(26,26,62,.1)" }}>

          {/* Tab switcher — matches screenshot pill tabs */}
          <div style={{ display: "flex", background: "#f0f0f6", borderRadius: 12, padding: 4, marginBottom: 32 }}>
            <button onClick={() => { setView("login"); setError(""); }} style={{ flex: 1, padding: "10px", borderRadius: 9, border: "none", background: isLogin ? "#ffffff" : "transparent", color: isLogin ? "#1a1a3e" : "#9ca3af", fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: isLogin ? "0 2px 8px rgba(26,26,62,.1)" : "none", transition: "all .2s" }}>Login</button>
            <button onClick={() => { setView("register"); setError(""); }} style={{ flex: 1, padding: "10px", borderRadius: 9, border: "none", background: !isLogin ? "#ffffff" : "transparent", color: !isLogin ? "#1a1a3e" : "#9ca3af", fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: !isLogin ? "0 2px 8px rgba(26,26,62,.1)" : "none", transition: "all .2s" }}>Register</button>
          </div>

          {/* Header */}
          <div style={{ marginBottom: 32 }}>
            <h2 style={{ fontSize: 28, fontWeight: 800, color: "#1a1a3e", letterSpacing: "-0.5px", marginBottom: 8, fontFamily: "'DM Sans', sans-serif" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 18, marginBottom: 20 }}>
            {!isLogin && (
              <div>
                ...
                <input name="name" value={form.name} onChange={handle} onKeyDown={e => e.key === "Enter" && submit()} placeholder="John Smith" className="sq-inp" style={{ ...inp }} />
              </div>
            )}
            <div>
              <label style={{ ...lbl }}>Email Address</label>
              <input name="email" type="email" value={form.email} onChange={handle} onKeyDown={e => e.key === "Enter" && submit()} placeholder="you@example.com" className="sq-inp" style={{ ...inp }} />
            </div>
            <div>
              <label style={{ ...lbl }}>Password</label>
              <input name="password" type="password" value={form.password} onChange={handle} onKeyDown={e => e.key === "Enter" && submit()} placeholder="••••••••••" className="sq-inp" style={{ ...inp }} />
            </div>
            {!isLogin && (
              <div>
                <label style={{ ...lbl }}>Account Role</label>
                <select name="role" value={form.role} onChange={handle} className="sq-inp" style={{ ...inp, background: "#fff" }}>
          ...
          {/* Alert */}
          {error && (
            <div style={{ padding: "12px 15px", borderRadius: 10, marginBottom: 18, background: isSuccess ? "rgba(16,185,129,.1)" : "rgba(239,68,68,.1)", border: `1px solid ${isSuccess ? "rgba(16,185,129,.3)" : "rgba(239,68,68,.3)"}`, color: isSuccess ? "#059669" : "#dc2626", fontSize: 13, fontWeight: 500 }}>
              {isSuccess ? "✓" : "⚠"} {errorMsg}
            </div>
          )}

          {/* Submit — dark navy button matching screenshot */}
          <button onClick={submit} disabled={loading} className="sq-btn-primary" style={{ width: "100%", padding: "15px", borderRadius: 12, border: "none", background: "#1a1a3e", color: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", boxShadow: "0 8px 24px rgba(26,26,62,.3)", opacity: loading ? 0.7 : 1 }}>
          ...
          <p style={{ textAlign: "center", fontSize: 13, color: "#9ca3af", marginTop: 20 }}>
            ...
            <span onClick={() => { setView(isLogin ? "register" : "login"); setError(""); }} className="sq-link" style={{ color: "#1a1a3e", fontWeight: 700, cursor: "pointer" }}>
...
function HomePage({ onLogin, onRegister, activeCount }) {
  ...
  return (
    /* Changed: main bg from #0a0818 → #f0f0f6 (light), sections use dark navy panels */
    <div style={{ minHeight: "100vh", background: "#f0f0f6", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      {/* Background orbs — adjusted for light bg */}
      <div style={{ position: "fixed", inset: 0, overflow: "hidden", pointerEvents: "none", zIndex: 0 }}>
        <div style={{ position: "absolute", width: 700, height: 700, borderRadius: "50%", background: "radial-gradient(circle, rgba(30,30,74,.06) 0%, transparent 65%)", top: "-200px", right: "-200px", animation: "orb1 15s ease infinite" }} />
        <div style={{ position: "absolute", width: 600, height: 600, borderRadius: "50%", background: "radial-gradient(circle, rgba(247,151,30,.05) 0%, transparent 65%)", bottom: "100px", left: "-150px", animation: "orb2 20s ease infinite" }} />
        <div style={{ position: "absolute", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, rgba(26,26,62,.04) 0%, transparent 65%)", top: "40%", left: "45%", animation: "orb1 18s ease infinite reverse" }} />
      </div>

      {/* ─── NAVBAR — dark navy as in screenshot ─── */}
      <nav style={{ position: "sticky", top: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 60px", background: "#1a1a3e", backdropFilter: "blur(20px)", borderBottom: "1px solid rgba(255,255,255,.06)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button onClick={onRegister} className="sq-nav-btn" style={{ padding: "9px 22px", borderRadius: 10, border: "none", background: "linear-gradient(135deg, #ffd200, #f7971e)", color: "#1a1a3e", fontSize: 14, fontWeight: 700, cursor: "pointer", boxShadow: "0 4px 14px rgba(247,151,30,.35)" }}>Get Started Free</button>
        </div>
      </nav>

      {/* ─── HERO — dark navy bg, light text ─── */}
      <section style={{ position: "relative", zIndex: 5, minHeight: "92vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 24px 60px", textAlign: "center", background: "#1a1a3e" }}>
        ...
        <h1 style={{ fontSize: "clamp(38px, 5.5vw, 68px)", fontWeight: 900, color: "#fff", letterSpacing: "-2px", lineHeight: 1.08, marginBottom: 28, fontFamily: "'DM Sans', sans-serif", maxWidth: 820 }}>
          ...
          <span className="hero-word" style={{ display: "inline-block", background: "linear-gradient(90deg, #ffd200, #f7971e, #ffd200)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundSize: "200% auto", animation: "fadeUp .7s ease both, shimmer 4s linear infinite" }}>Smarter.</span>
          <br />
        ...
        <p style={{ fontSize: "clamp(16px, 2.5vw, 20px)", color: "rgba(255,255,255,.55)", maxWidth: 580, lineHeight: 1.7, marginBottom: 48, animation: "fadeUp .7s ease .35s both" }}>
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap", justifyContent: "center", animation: "fadeUp .7s ease .5s both" }}>
          <button onClick={onRegister} className="sq-btn-primary" style={{ padding: "16px 36px", borderRadius: 14, border: "none", background: "linear-gradient(135deg, #ffd200, #f7971e)", color: "#1a1a3e", fontSize: 16, fontWeight: 700, cursor: "pointer", boxShadow: "0 8px 28px rgba(247,151,30,.4)" }}>
            Start for Free →
          </button>
        ...
        <div style={{ display: "flex", gap: "clamp(24px, 6vw, 72px)", marginTop: 72, flexWrap: "wrap", justifyContent: "center", animation: "fadeUp .7s ease .65s both" }}>
          {stats.map(s => (
            <div key={s.value} className="sq-stat" style={{ textAlign: "center" }}>
              <div style={{ fontSize: "clamp(28px, 4vw, 42px)", fontWeight: 900, color: "#ffd200", letterSpacing: "-1.5px", fontFamily: "'DM Sans', sans-serif", lineHeight: 1 }}>{s.value}</div>
              <div style={{ fontSize: 13, color: "rgba(255,255,255,.45)", marginTop: 6, fontWeight: 500, letterSpacing: "0.5px", textTransform: "uppercase" }}>{s.label}</div>
            </div>
          ))}
        </div>
      ...
      {/* ─── FEATURES — light bg section ─── */}
      <section style={{ position: "relative", zIndex: 5, padding: "100px 24px", background: "#f0f0f6" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          {/* Section header */}
          <div style={{ textAlign: "center", marginBottom: 60 }}>
            <div style={{ display: "inline-block", padding: "5px 16px", borderRadius: 20, background: "rgba(26,26,62,.08)", border: "1px solid rgba(26,26,62,.15)", color: "#1a1a3e", fontSize: 12, fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", marginBottom: 20 }}>Features</div>
            <h2 style={{ fontSize: "clamp(24px, 3.5vw, 38px)", fontWeight: 800, color: "#1a1a3e", letterSpacing: "-1px", fontFamily: "'DM Sans', sans-serif" }}>Everything you need to run<br /><span style={{ color: "#f7971e" }}>flawless queues</span></h2>
          </div>

          {/* Feature grid */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 24 }}>
            {features.map(f => (
              <div key={f.title} className="sq-feat" style={{ padding: "32px 28px", borderRadius: 20, background: "#ffffff", border: "1px solid #e2e2ee", boxShadow: "0 2px 16px rgba(26,26,62,.06)" }}>
                <div style={{ width: 52, height: 52, borderRadius: 16, background: "rgba(26,26,62,.07)", border: "1px solid rgba(26,26,62,.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, marginBottom: 20 }}>{f.icon}</div>
                <h3 style={{ fontSize: 18, fontWeight: 700, color: "#1a1a3e", marginBottom: 10, letterSpacing: "-0.3px" }}>{f.title}</h3>
      ...
      <section style={{ position: "relative", zIndex: 5, padding: "80px 24px", background: "#1a1a3e", borderTop: "1px solid rgba(255,255,255,.05)", borderBottom: "1px solid rgba(255,255,255,.05)" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            {steps.map((s, i) => (
              <div key={s.n} className="sq-step" style={{ display: "flex", alignItems: "flex-start", gap: 28, padding: "28px 32px", borderRadius: 18, background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)" }}>
                <div style={{ fontSize: "clamp(26px, 3.5vw, 36px)", fontWeight: 900, color: i === 0 ? "#ffd200" : i === 1 ? "#fcd34d" : "#10b981", fontFamily: "'DM Sans', sans-serif", opacity: 0.9, flexShrink: 0, lineHeight: 1 }}>{s.n}</div>
                <div>
      ...
      {/* ─── CTA BANNER — light bg with dark navy card ─── */}
      <section style={{ position: "relative", zIndex: 5, padding: "100px 24px", textAlign: "center", background: "#f0f0f6" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", padding: "60px 48px", borderRadius: 28, background: "#1a1a3e", border: "1px solid rgba(255,255,255,.06)", boxShadow: "0 20px 60px rgba(26,26,62,.2)" }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <button onClick={onRegister} className="sq-btn-primary" style={{ padding: "15px 32px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #ffd200, #f7971e)", color: "#1a1a3e", fontSize: 15, fontWeight: 700, cursor: "pointer", boxShadow: "0 8px 24px rgba(247,151,30,.35)" }}>
              Create Free Account
            </button>
      ...
      {/* ─── FOOTER — dark navy bg ─── */}
      <footer style={{ position: "relative", zIndex: 5, background: "#13132e", borderTop: "1px solid rgba(255,255,255,.06)", padding: "40px 60px 32px", fontFamily: "'DM Sans', sans-serif" }}>
        {/* Top row */}
