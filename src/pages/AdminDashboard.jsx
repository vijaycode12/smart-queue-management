...
import {
  ...
  updateQueueStatus, deleteQueue          // ← NEW imports
} from "../services/api";
...
export default function AdminDashboard({ user, onLogout }) {
  ...
  const handleCreateQueue = async () => {
    ...
    try {
      ...
      showToast(e.response?.data || "Failed to create", "error");
    }
    setLoading(false);
  };

  // ACTIVE → PAUSED, PAUSED → ACTIVE, INACTIVE → ACTIVE
  const handleToggleStatus = async (queue) => {
    ...
    try {
      ...
      showToast(`Queue "${queue.queueName}" is now ${newStatus}`);
      fetchAll();
    } catch (e) {
  ...
  const handleDeleteQueue = async () => {
    ...
    try {
      ...
      showToast(`Queue "${deleteConfirm.name}" deleted successfully`);
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 14, color: "#9ca3af", margin: 0 }}>
              {activeTab === "overview" ? "Real-time queue monitoring and control"
               : activeTab === "queues"  ? "Create, pause, deactivate and delete queues"
        ...
        <div style={{ flex: 1, padding: "36px 40px", overflowY: "auto" }}>
              ...
              {queues.length === 0 ? (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 20 }}>
                  {queues.map(q => {
                    ...
                    const qs       = queueStatusStyle(q.status);
                    ...
                    return (
                      <div key={q.id} style={{
                        ...
                        {/* Status Banners */}
                        {isPaused && (
                          <div style={{ background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 10, padding: "10px 14px", marginBottom: 12, fontSize: 13, color: "#d97706", fontWeight: 600, textAlign: "center" }}>
                            ⏸ Queue is paused — no new tokens can be issued
                          </div>
                        )}
                        {isInactive && (
                          <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 10, padding: "10px 14px", marginBottom: 12, fontSize: 13, color: "#dc2626", fontWeight: 600, textAlign: "center" }}>
                            🚫 This queue is inactive
                          </div>
                        )}
                        ...
                        {serving && !isInactive ? (
                          <div style={{ background: "linear-gradient(135deg, #1e1b4b, #302b63)", borderRadius: 16, padding: "16px 20px", marginBottom: 12 }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                              ...
                              <span style={{ fontSize: 26, fontWeight: 900, color: "#ffd200", letterSpacing: "-1px" }}>#{String(serving.tokenNumber).padStart(3, "0")}</span>
                            </div>
                        ...
                        {tokens.slice(0, 4).map(t => {
                          const c = tokenStatusStyle(t.status);
                          return (
          ...
          {activeTab === "queues" && (
            <>
              {/* Create Form */}
              <div style={{ background: "#fff", borderRadius: 20, padding: "28px 32px", border: "1px solid #e5e7eb", boxShadow: "0 2px 8px rgba(0,0,0,0.04)", marginBottom: 28 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 24, marginBottom: 20, padding: "14px 20px", background: "#fff", borderRadius: 14, border: "1px solid #e5e7eb", flexWrap: "wrap" }}>
                ...
                {[
                  ...
                  { label: "Inactive", bg: "#fef2f2", color: "#dc2626", dot: "#ef4444", desc: "Permanently closed"    },
                ].map(s => (
              ...
              <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #e5e7eb", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", padding: "14px 28px", background: "#f8fafc", borderBottom: "2px solid #e5e7eb", fontSize: 11, fontWeight: 800, color: "#9ca3af", letterSpacing: 1, textTransform: "uppercase" }}>
                  ...
                  <span style={{ width: 170 }}>Created At</span>
                  <span style={{ width: 360 }}>Actions</span>
                </div>
                ...
                {queues.length === 0 ? (
                  ...
                  return (
                    <div key={q.id} style={{ display: "flex", alignItems: "center", padding: "16px 28px", borderBottom: "1px solid #f1f5f9", background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                      ...
                      <span style={{ width: 60, fontSize: 13, color: "#9ca3af", fontWeight: 700 }}>#{q.id}</span>
                      ...
                      <span style={{ width: 170, fontSize: 13, color: "#9ca3af" }}>
                        {q.createdAt ? q.createdAt.slice(0, 16).replace("T", " ") : "—"}
                      </span>

                      {/* ── Action Buttons ── */}
                      <span style={{ width: 360, display: "flex", gap: 8, flexWrap: "wrap" }}>

                        {/* Pause button — shown when ACTIVE */}
                        {q.status === "ACTIVE" && (
                          <button onClick={() => handleToggleStatus(q)} style={{ padding: "7px 14px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700, background: "#fffbeb", color: "#d97706" }}>
                            ⏸ Pause
                          </button>
                        )}
                        ...
                        {q.status === "PAUSED" && (
                          <button onClick={() => handleToggleStatus(q)} style={{ padding: "7px 14px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700, background: "#f0fdf4", color: "#16a34a" }}>
                            ▶ Resume
                          </button>
                        )}
                        ...
                        {/* Deactivate — shown when ACTIVE or PAUSED */}
                        {q.status !== "INACTIVE" && (
                          <button onClick={() => handleSetInactive(q)} style={{ padding: "7px 14px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700, background: "#fef2f2", color: "#dc2626" }}>
                            🚫 Deactivate
                          </button>
                        )}

                        {/* Call Next — only when ACTIVE */}
                        {q.status === "ACTIVE" && (
                          <button onClick={() => handleCallNext(q.id)} style={{ padding: "7px 14px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700, background: "#eef2ff", color: "#6366f1" }}>
                            📢 Call Next
                          </button>
                        )}

                        {/* Delete — always visible, opens modal */}
                        <button onClick={() => setDeleteConfirm({ id: q.id, name: q.queueName })} style={{ padding: "7px 14px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 700, background: "#fff1f2", color: "#be123c" }}>
                          🗑 Delete
                        </button>
      ...
      {deleteConfirm && (
        <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#fff", borderRadius: 24, padding: "40px", width: 440, boxShadow: "0 24px 64px rgba(0,0,0,0.25)", fontFamily: "'DM Sans', sans-serif" }}>
            ...
            <h3 style={{ fontSize: 22, fontWeight: 900, color: "#111", margin: "0 0 10px", textAlign: "center", letterSpacing: "-0.5px" }}>Delete Queue?</h3>
            <p style={{ fontSize: 15, color: "#6b7280", textAlign: "center", margin: "0 0 8px" }}>You are about to permanently delete:</p>
            <p style={{ fontSize: 16, fontWeight: 800, color: "#111", textAlign: "center", margin: "0 0 20px", padding: "12px 20px", background: "#f8fafc", borderRadius: 12 }}>
              "{deleteConfirm.name}"
            </p>
            <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 12, padding: "12px 16px", marginBottom: 28, fontSize: 13, color: "#dc2626", fontWeight: 500 }}>
              ⚠️ This action cannot be undone. All tokens linked to this queue may also be affected.
            </div>
            <div style={{ display: "flex", gap: 12 }}>
              <button onClick={() => setDeleteConfirm(null)} style={{ flex: 1, padding: "14px", borderRadius: 12, border: "1.5px solid #e5e7eb", background: "#fff", fontSize: 15, fontWeight: 700, cursor: "pointer", color: "#374151" }}>
                Cancel
              </button>
              <button onClick={handleDeleteQueue} style={{ flex: 1, padding: "14px", borderRadius: 12, border: "none", background: "linear-gradient(135deg, #dc2626, #b91c1c)", color: "#fff", fontSize: 15, fontWeight: 800, cursor: "pointer" }}>
                Yes, Delete
              </button>
            </div>
          </div>
